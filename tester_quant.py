...
class QuantizedTester:
    def __init__(self, config):
        # Set Device
        self.m_cfg = config["model"]
        self.t_cfg = config["train"]
        self.quant_cfg = config["quant"]
        self.device = get_device()

        # Download or load dataset
        train_path, _, test_path = prepare_files()
        ...
        # Quantizer
        manager = QuantizationManager(device=self.device)
        
        # Load quantized model
        quant_checkpoint_path = self.quant_cfg["checkpoint_path"]
        if not os.path.exists(quant_checkpoint_path):
            raise FileNotFoundError(f"[ERROR] Quantized checkpoint not found: {quant_checkpoint_path}")
        self.model = manager.load(blank_model, quant_checkpoint_path)
